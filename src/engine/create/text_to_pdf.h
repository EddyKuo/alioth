...
//
// 非 ASCII 走內嵌 CJK 字型子集（ADR-007）。標準 14 字型是 WinAnsiEncoding，
// 畫不出 CJK，所以含非 ASCII 的輸入改由 CjkFontLibrary 量測與內嵌，版面規則
// 仍走 domain 的同一份 detail::paginate，只有字寬來源不同。
//
// 界線是「子集裡有這個字形」：找不到字型、或字型裡缺這個字，一律明確失敗，
// 不退回拉丁字型。靜默輸出的後果是一整頁的空白或方框，使用者無從得知是
// 內容沒進去還是字型沒裝——SDD §7 要求降級必須看得見。
