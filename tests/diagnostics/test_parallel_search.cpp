// 並行搜尋——**缺陷重現器，不是產品行為測試**。
//
// 這支程式會間歇性堆積毀損（0xc0000374）或 SEGFAULT。那不是它壞了，那就是
// 它要記錄的現象本身：PDFium 的文件把手可並存、不可並行
// （EXC_20260905_RD_SA_parallel_search、ADR-005）。
//
// 因此它不登錄成 CTest 測試，只在需要重現時手動跑。產品端真正該守的規則是
// 「不得接線任何並行 PDFium 路徑」，那條由 tools/check_layering.py 靜態檢查。
//
// 這組測試存在的直接原因是一個量出來的數字：單執行緒搜尋 500 頁 A0 工程圖語料
// 要 3874 毫秒，而 PRD-SRCH-001 的預算是 2000 毫秒。並行看起來是解法，但並行的
// 正確性比循序難驗——漏頁、重複回報、計數歸不了零導致永遠不結束，三者都不會崩潰。
// 結論是改用單一擷取器預建文字索引（PRD-SRCH-001，ADR-005），不需要並行。
